...
func (s *UserUsecaseTestSuite) TestFindUserByUsername_Success() {
	username := "test"
	expectedUser := model.UserCredential{Id: 1, Username: username, Role: "user"}

	s.repoMock.On("GetByUsername", username).Return(expectedUser, nil)

	actualUser, err := s.usecase.FindUserByUsername(username)
...
func (s *UserUsecaseTestSuite) TestFindUserByUsername_NotFound() {
	username := "notfound"
	expectedError := errors.New("user not found")

	s.repoMock.On("GetByUsername", username).Return(model.UserCredential{}, expectedError)
...
func (s *UserUsecaseTestSuite) TestFindUserByUsernamePassword_Success() {
	username := "test"
	password := "password"
	expectedUser := model.UserCredential{Id: 1, Username: username, Password: password, Role: "user"}
